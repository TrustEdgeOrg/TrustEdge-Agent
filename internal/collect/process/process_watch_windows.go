...
func (w *windowsProcessWatcher) Run(ctx context.Context) <-chan collect.Change {
...
func (w *windowsProcessWatcher) run(ctx context.Context, out chan<- collect.Change) error {
...
func changeFromETW(e *etw.Event) (collect.Change, bool) {
	if e == nil {
		return collect.Change{}, false
	}
	switch e.Header.ID {
	case 1:
...
func processStartFromETW(e *etw.Event) (collect.Change, bool) {
	...
	return collect.Change{Type: constants.TypeProcessStart, Payload: processPayload(row)}, true
}

func processStopFromETW(e *etw.Event) (collect.Change, bool) {
	props, err := e.EventProperties()
	if err != nil {
		return collect.Change{}, false
	}
	...
	comm := filepath.Base(image)
	row := processRow{PID: pid, PPID: ppid, Comm: comm, Executable: image, Cmdline: cmdline}
	return collect.Change{Type: constants.TypeProcessExit, Payload: processPayload(row)}, true
}
