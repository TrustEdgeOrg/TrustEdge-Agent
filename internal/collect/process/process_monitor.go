...
func (m *ProcessMonitor) Observe(c collect.Change) bool {
...
func (m *ProcessMonitor) Poll() []collect.Change {
...
func applyPollChanges(seen map[int]processRow, current map[int]processRow, changes []collect.Change, capped bool) {
	if !capped {
		for k := range seen {
			delete(seen, k)
		}
