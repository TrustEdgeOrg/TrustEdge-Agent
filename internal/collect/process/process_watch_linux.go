...
func (w *linuxProcessWatcher) Run(ctx context.Context) <-chan collect.Change {
...
func (w *linuxProcessWatcher) run(ctx context.Context, out chan<- collect.Change) error {
...
func parseProcMessages(buf []byte) []collect.Change {
	var out []collect.Change
...
func procEventChange(ev []byte) (collect.Change, bool) {
	if len(ev) < 16 {
