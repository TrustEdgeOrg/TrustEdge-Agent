...
type stubProbe struct {
	app  *platform.ForegroundInfo
	idle float64
}

func (s stubProbe) OSVersion() string                       { return "test" }
func (s stubProbe) ForegroundApp() *platform.ForegroundInfo { return s.app }
func (s stubProbe) IdleSeconds() float64                    { return s.idle }

type sequenceProbe struct {
	apps []*platform.ForegroundInfo
	idx  int
	idle float64
}

func (s *sequenceProbe) OSVersion() string { return "test" }
func (s *sequenceProbe) ForegroundApp() *platform.ForegroundInfo {
	if len(s.apps) == 0 {
		return nil
	}
	if s.idx >= len(s.apps) {
...
func TestActionTrackerResetClearsWindow(t *testing.T) {
	probe := stubProbe{app: &platform.ForegroundInfo{Name: "Mail", BundleID: "com.mail"}}
