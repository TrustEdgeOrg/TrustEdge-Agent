...
func (a *Agent) Run(ctx context.Context) error {
	post := func(typ string, payload map[string]any) {
		ev := models.NewEvent(a.clock, a.deviceID, typ, payload)
		if err := a.postEvent(ev); err != nil {
			a.log.Printf("post %s: %v", typ, err)
			return
		}
		a.log.Printf("posted %s", typ)
	}

	post(constants.TypeClientDetails, a.collector.ClientDetailsPayload())
	...
	go func() {
		for change := range monitor.Run(ctx) {
			...
			post(constants.TypeNetworkSummary, a.collector.NetworkSummaryPayload())
		}
	}()
